...
unsigned int nStakeMinAge; /// min age 6 hour/
...
const string strMessageMagic = "BCPcoin Signed Message:\n";
...
unsigned int GetnStakeMinAge()
{
        nStakeMinAge  = 12 * 60 * 60; //12 hours
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    {
        nSubsidy = 1000 * COIN; //premine for SWAP old coins
    }
        else if(pindexBest->nHeight >= 1  && pindexBest->nHeight <= 200)
    {
        nSubsidy = 1000 * COIN;
    }
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && nHeight >  GetLastPowBlock())
