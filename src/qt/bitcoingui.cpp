...
BitcoinGUI::BitcoinGUI(QWidget *parent):
    ...
    nWeight(0)
{
    setFixedSize(980, 514);
    setWindowTitle(tr("BuyCoin") + " - " + tr("Wallet"));
    ...
    frameBlocks->setStyleSheet("QWidget { background-image:url(:images/bgb_free); margin-bottom: 5px; }");
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a BuyCoin address"));
    ...
    aboutAction = new QAction(tr("&About BuyCoin"), this);
    aboutAction->setToolTip(tr("Show information about BuyCoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for BuyCoin"));
...
void BitcoinGUI::createMenuBar()
{
    ...
    appMenuBar->setStyleSheet("background-image: url(:/images/bgb);background-repeat: no-repeat; ");
    // Configure the menus
    QMenu *file = appMenuBar->addMenu(tr("&File"));
    file->addAction(backupWalletAction);
    file->setStyleSheet("background-image: url(:/images/bkg);");
    ...
    settings->setStyleSheet("background-image: url(:/images/bkg);");
    ...
    help->setStyleSheet("background-image: url(:/images/bkg);");
    help->addSeparator();
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("BuyCoin client") + QString(" ") + tr("[testnet]"));
...
//    trayIconMenu->>setStyleSheet("QWidget { background-repeat: no-repeat; background-image: url(:/images/bkg); border-color: red; }");
    ...
    trayIcon->setToolTip(tr("BuyCoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to BuyCoin network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid BuyCoin address or malformed URI parameters."));
    }
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    else
    {
        ...
        else if (!nWeight || nBestHeight < GetLastPowBlock())
